...
public class OracleSqlUtilsTest
{
    ...
    public void buildSql_WithBasicQuery_GeneratesSelectFromQuery()
    {
        ...
        Constraints constraints = getConstraints(constraintMap, Collections.emptyList(), Constraints.DEFAULT_NO_LIMIT);
    ...
    public void buildSql_WithOrderBy_GeneratesQueryWithOrderByWithoutLimit()
    {
        ...
        Schema schema = makeSchema(constraintMap);
        Constraints constraints = getConstraints(constraintMap, orderByFields, Constraints.DEFAULT_NO_LIMIT);
    ...
    @Test
    public void buildSql_WithPartitionClause_GeneratesQueryWithPartitionInFromClause()
    {
        Schema schema = makeSchema(constraintMap);
        Constraints constraints = getConstraints(constraintMap, Collections.emptyList(), Constraints.DEFAULT_NO_LIMIT);

        Map<String, String> splitProperties = new LinkedHashMap<>();
        splitProperties.put("partition_key", "partition_value");
        ...
        when(splitWithPartition.getProperty("partition_key")).thenReturn("partition_value");

        String expectedSql = "SELECT null FROM \"test_schema\".\"test_table\" PARTITION (partition_value)";

        List<TypeAndValue> parameterValues = new ArrayList<>();
        String sql = OracleSqlUtils.buildSql(tableName, schema, constraints, splitWithPartition, parameterValues);
    ...
    @Test
    public void buildSql_WithAllPartitions_GeneratesQueryWithoutPartitionClause()
    {
        Schema schema = makeSchema(constraintMap);
        Constraints constraints = getConstraints(constraintMap, Collections.emptyList(), Constraints.DEFAULT_NO_LIMIT);
        ...
        List<TypeAndValue> parameterValues = new ArrayList<>();
        String sql = OracleSqlUtils.buildSql(tableName, schema, constraints, splitWithAllPartitions, parameterValues);
    ...
    @Test
    public void buildSql_WithLimit_GeneratesQueryWithFetchFirst()
    {
        Schema schema = makeSchema(constraintMap);
        Constraints constraints = getConstraints(constraintMap, Collections.emptyList(), 100);

        String expectedSql = "SELECT null FROM \"test_schema\".\"test_table\"  FETCH FIRST 100 ROWS ONLY";

        List<TypeAndValue> parameterValues = new ArrayList<>();
        String sql = OracleSqlUtils.buildSql(tableName, schema, constraints, split, parameterValues);
    ...
    public void buildSql_WithEmptySchema_GeneratesQueryWithNull()
    {
        ...
        List<TypeAndValue> parameterValues = new ArrayList<>();
        String sql = OracleSqlUtils.buildSql(tableName, emptySchema, constraints, split, parameterValues);
