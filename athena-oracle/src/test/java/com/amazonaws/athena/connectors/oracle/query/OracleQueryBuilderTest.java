...
public class OracleQueryBuilderTest
{
    ...
    @Test
    public void build_WithLimitClause_IncludesFetchFirstInSql()
    {
        ...
        assertNotNull("SQL should not be null", sql);
        assertTrue("SQL should contain FETCH FIRST clause", sql.contains("FETCH FIRST"));
        assertTrue("SQL should contain ROWS ONLY", sql.contains("ROWS ONLY"));
        assertTrue("SQL should contain limit value", sql.contains("50"));
    }

    @Test
    public void build_WithNoLimit_DoesNotIncludeFetchFirst()
    {
        Constraints constraints = createConstraintsWithOrderBy(Collections.emptyList());
        ...
        assertNotNull("SQL should not be null", sql);
        assertFalse("SQL should not contain FETCH FIRST when limit is 0", sql.contains("FETCH FIRST"));
    }
    ...
    @Test
    public void build_WithPartitionClause_GeneratesQueryWithPartitionInFrom()
    {
        Map<String, String> splitProperties = new HashMap<>();
        splitProperties.put(BLOCK_PARTITION_COLUMN_NAME, "p0");
        splitProperties.put("p0", "partition_value");
        Split splitWithPartition = mock(Split.class);
        when(splitWithPartition.getProperties()).thenReturn(splitProperties);
        when(splitWithPartition.getProperty(BLOCK_PARTITION_COLUMN_NAME)).thenReturn("p0");
        when(splitWithPartition.getProperty("p0")).thenReturn("partition_value");
        ...
        assertNotNull("SQL should not be null", sql);
        assertTrue("SQL should contain PARTITION clause", sql.contains("PARTITION"));
        assertTrue("SQL should contain partition value", sql.contains("partition_value"));
    }
    ...
    {
        Map<String, String> splitProperties = new HashMap<>();
        splitProperties.put(BLOCK_PARTITION_COLUMN_NAME, ALL_PARTITIONS);
        Split splitWithAllPartitions = mock(Split.class);
        when(splitWithAllPartitions.getProperties()).thenReturn(splitProperties);
        when(splitWithAllPartitions.getProperty(BLOCK_PARTITION_COLUMN_NAME)).thenReturn(ALL_PARTITIONS);
        ...
        assertNotNull("SQL should not be null", sql);
        assertFalse("SQL should not contain PARTITION clause for ALL_PARTITIONS", sql.contains("PARTITION"));
    }
    ...
    public void build_WithAllComponents_GeneratesCompleteQuery()
    {
        ...
        OracleQueryBuilder builder = queryFactory.createQueryBuilder();
        builder.withCatalog(null); // Oracle uses catalog.schema.table, but catalog is typically null
        builder.withProjection(testSchema, split);
        builder.withTableName(TEST_TABLE);
        builder.withOrderByClause(constraints);
        builder.withLimitClause(constraints);
    ...
    @Test
    public void getPartitionClause_WhenSet_ReturnsPartitionClause()
    {
        Map<String, String> splitProperties = new HashMap<>();
        splitProperties.put(BLOCK_PARTITION_COLUMN_NAME, "p0");
        splitProperties.put("p0", "partition_value");
        ...
        String partitionClause = builder.getPartitionClause();
        assertNotNull("Partition clause should not be null", partitionClause);
        assertTrue("Partition clause should contain PARTITION", partitionClause.contains("PARTITION"));
        assertTrue("Partition clause should contain partition value", partitionClause.contains("partition_value"));
    }

    @Test(expected = NullPointerException.class)
    public void OracleQueryBuilder_WhenCreatedWithNullTemplate_ThrowsNullPointerException()
    {
        new OracleQueryBuilder(null);
    }
    ...
    public void build_WithNullCatalog_DoesNotIncludeCatalogInFromClause()
    {
        ...
        assertFalse("SQL should not contain catalog when null", sql.contains("null."));
    }
