...
public class MySqlSqlUtilsTest
{
    ...
    @Test
    public void buildSql_WithOrderBy_GeneratesQueryWithOrderByAndLimit()
    {
        Map<String, ValueSet> constraintMap = new LinkedHashMap<>();
        ValueSet rangeSet = SortedRangeSet.newBuilder(INT_TYPE, false)
                .add(new Range(Marker.exactly(allocator, INT_TYPE, 10), Marker.exactly(allocator, INT_TYPE, 10)))
        ...
        Schema schema = makeSchema(constraintMap);
        Constraints constraints = new Constraints(constraintMap, Collections.emptyList(), orderByFields, 100, Collections.emptyMap(), null);
        
        List<TypeAndValue> expectedParams = new ArrayList<>();
        expectedParams.add(new TypeAndValue(INT_TYPE, 10));
        String expectedSql = "SELECT `intCol` FROM `test_schema`.`test_table`  WHERE (`intCol` = ?) ORDER BY `intCol` ASC, `stringCol` DESC LIMIT 100";
        
        executeAndVerify(constraints, schema, expectedParams, expectedSql);
    }

    @Test
    public void buildSql_WithOrderByNullsLast_GeneratesQueryWithIsNull()
    {
        Map<String, ValueSet> constraintMap = new LinkedHashMap<>();
        ValueSet rangeSet = SortedRangeSet.newBuilder(INT_TYPE, false)
                .add(new Range(Marker.exactly(allocator, INT_TYPE, 10), Marker.exactly(allocator, INT_TYPE, 10)))
        ...
        List<OrderByField> orderByFields = new ArrayList<>();
        orderByFields.add(new OrderByField("intCol", OrderByField.Direction.ASC_NULLS_LAST));
        ...
        List<TypeAndValue> expectedParams = new ArrayList<>();
        expectedParams.add(new TypeAndValue(INT_TYPE, 10));
        String expectedSql = "SELECT `intCol` FROM `test_schema`.`test_table`  WHERE (`intCol` = ?) ORDER BY ISNULL(`intCol`) ASC, `intCol` ASC";
    ...
    public void buildSql_WithPartitionClause_GeneratesQueryWithPartition()
    {
        ...
        String expectedSql = "SELECT null FROM `test_schema`.`test_table` PARTITION(p0)";

        List<TypeAndValue> parameterValues = new ArrayList<>();
        String sql = MySqlSqlUtils.buildSql(tableName, schema, constraints, splitWithPartition, parameterValues);
    ...
    public void buildSql_WithAllPartitions_DoesNotIncludePartitionClause()
    {
        ...
        // Create split with all partitions
        Map<String, String> splitProperties = new LinkedHashMap<>();
        ...
        List<TypeAndValue> parameterValues = new ArrayList<>();
        String sql = MySqlSqlUtils.buildSql(tableName, schema, constraints, splitWithAllPartitions, parameterValues);
    ...
    public void buildSql_WithEmptySchema_GeneratesQueryWithNull()
    {
        ...
        List<TypeAndValue> parameterValues = new ArrayList<>();
        String sql = MySqlSqlUtils.buildSql(tableName, emptySchema, constraints, split, parameterValues);
    ...
    public void buildSql_WithLimit_GeneratesQueryWithLimit()
    {
        Map<String, ValueSet> constraintMap = new LinkedHashMap<>();
        Schema schema = makeSchema(constraintMap);
        Constraints constraints = new Constraints(constraintMap, Collections.emptyList(), Collections.emptyList(), 50, Collections.emptyMap(), null);

        String expectedSql = "SELECT null FROM `test_schema`.`test_table`   LIMIT 50";
