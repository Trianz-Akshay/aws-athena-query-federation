...
public class MySqlQueryBuilderTest
{
    ...
    public void build_WithProjectionAndTableName_GeneratesCorrectSelectQuery()
    {
        ...
        assertFalse("Partition column should be excluded", sql.contains("`partition_name`"));
    }
    ...
    public void build_WithOrderByClause_GeneratesQueryWithOrderBy()
    {
        ...
        // MySQL uses ISNULL() for NULLS LAST
        assertTrue("SQL should contain name ordering", sql.contains("`name` ASC"));
        assertTrue("SQL should contain score ordering", sql.contains("`score` DESC"));
    }

    @Test
    public void build_WithOrderByNullsLast_GeneratesQueryWithIsNull()
    {
        List<OrderByField> orderByFields = new ArrayList<>();
        orderByFields.add(new OrderByField(COLUMN_NAME, OrderByField.Direction.ASC_NULLS_LAST));
        Constraints constraints = createConstraintsWithOrderBy(orderByFields);
        ...
        assertNotNull("SQL should not be null", sql);
        assertTrue("SQL should contain ORDER BY clause", sql.contains("ORDER BY"));
        assertTrue("SQL should contain ISNULL for NULLS LAST", sql.contains("ISNULL(`name`)"));
    }

    @Test
    public void build_WithOrderByNullsFirst_GeneratesQueryWithIsNull()
    {
        List<OrderByField> orderByFields = new ArrayList<>();
        orderByFields.add(new OrderByField(COLUMN_NAME, OrderByField.Direction.DESC_NULLS_FIRST));
        Constraints constraints = createConstraintsWithOrderBy(orderByFields);
        ...
        assertNotNull("SQL should not be null", sql);
        assertTrue("SQL should contain ORDER BY clause", sql.contains("ORDER BY"));
        assertTrue("SQL should contain ISNULL for NULLS FIRST", sql.contains("ISNULL(`name`)"));
    }
    ...
    public void build_WithLimitClause_IncludesLimitInSql()
    {
        Constraints constraints = new Constraints(new HashMap<>(), Collections.emptyList(),
                Collections.emptyList(), 100, Collections.emptyMap(), null);
    ...
    @Test
    public void build_WithPartitionClause_IncludesPartitionInFromClause()
    {
        ...
        assertNotNull("SQL should not be null", sql);
        assertTrue("SQL should contain PARTITION clause", sql.contains("PARTITION(p0)"));
    }
    ...
    public void build_WithAllComponents_GeneratesCompleteQuery()
    {
        List<OrderByField> orderByFields = createOrderByFields();
        Constraints constraints = new Constraints(new HashMap<>(), Collections.emptyList(),
                orderByFields, 100, Collections.emptyMap(), null);
    ...
    public void withProjection_WithPartitionColumn_FiltersOutPartitionColumn()
    {
        ...
        assertFalse("Partition column should be filtered out", projection.contains("`partition_name`"));
        assertEquals("Projection should have 4 columns (excluding partition)", 4, projection.size());
    }

    @Test(expected = NullPointerException.class)
    public void MySqlQueryBuilder_WhenCreatedWithNullTemplate_ThrowsNullPointerException()
    {
        new MySqlQueryBuilder(null);
    }
    ...
    private Schema createTestSchema()
    {
        ...
        fields.add(new Field(PARTITION_COLUMN, new FieldType(true, STRING_TYPE, null), null));
